...
export function updateCdd(previousData: CdmData, nextData: CdmData, updatePartialCddState?: PartialCddState): CdmData {
	const dg = nextData.documentGraph;
	const nextChangeCounter = nextData.changeLog.changeCounter;
	const previousCddState = previousData.cddState;

	const cddState = reduceCddState(dg, nextChangeCounter, previousCddState, updatePartialCddState);
