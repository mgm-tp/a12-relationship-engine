...
class FileSystemModelLoader implements ModelLoader {
	...
	async load(descriptors: Model.Descriptor[]): Promise<ModelMap> {
		return descriptors.reduce((modelMap, { name, modelType }) => {
			return {
				...modelMap,
				[name]:
					modelType === "document"
						? readDocumentAndValidationModel(name)
						: modelType === "form"
							? readFormModel(name)
							: readModel(name)
			};
...
export const fileSystemModelLoader = new FileSystemModelLoader();
