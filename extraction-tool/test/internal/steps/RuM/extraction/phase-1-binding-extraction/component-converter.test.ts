...
describe("convertComponents", () => {
	it("should build DualPaneSelection with availableItemsOverviewModel from candidate use", () => {
		...
		expect(result.componentType).toBe("DualPaneSelection");
		expect(result.availableItemsOverviewModel).toBe("CandidateOverview");
		expect(result.selectedItemsOverviewModel).toBe("SelectedOverview");
	});

	it("should build DualPaneSelection with candidatePageSize", () => {
		const comp = makeLegacyComponent("DualPaneSelection", {
			models: [
				{ name: "CandidateOverview", use: "candidate" },
				{ name: "SelectedOverview", use: "link" }
	...
	it("should map link use + overview modelType to selectedItemsOverviewModel", () => {
		const comp = makeLegacyComponent("DualPaneSelection", {
			models: [
				{ name: "CandidateOverview", use: "candidate" },
				{ name: "SelectedOverview", use: "link" }
	...
	it("should map link use + form modelType to linkFormModel", () => {
		const comp = makeLegacyComponent("DualPaneSelection", {
			models: [
				{ name: "CandidateOverview", use: "candidate" },
		...
		// Make LinkForm have form modelType
		const header = (context.formModel as Record<string, unknown>).header as Record<string, unknown>;
		header.modelReferences = [
			{ reference: "CandidateOverview", modelType: "overview", purpose: "overview" },
			{ reference: "LinkForm", modelType: "form", purpose: "form" }
		];
		const result = convertComponents(kind, context);

		expect(result.linkFormModel).toBe("LinkForm");
		expect(result.selectedItemsOverviewModel).toBeUndefined();
	});

	it("should build TableList component with linkPageSize", () => {
	...
	it("should not emit pane-label fields even when DualPane props contain pane labels", () => {
		const comp = makeLegacyComponent("DualPaneSelection", {
			models: [
				{ name: "CandidateOverview", use: "candidate" },
				{ name: "SelectedOverview", use: "link" }
		...
		expect(result).not.toHaveProperty("availableItemsLabel");
		expect(result).not.toHaveProperty("selectedItemsLabel");
	});
});
