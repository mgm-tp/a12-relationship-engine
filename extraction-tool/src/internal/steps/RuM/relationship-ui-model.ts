 ...
 *
 * This model defines how a form element configures the UI for relationship data,
...
export namespace RelationshipUiModel {
	/**
	 * The content of a relationship UI model.
	 */
	...
	export interface ComponentConfiguration {
		...
		/**
		 * Reference to the overview model for displaying candidate items.
		...
		/**
		 * Reference to the query model for fetching candidate items.
		 ...
		 * Only applicable when componentType is "DropDownSelection".
		 * This query should include a variable placeholder (e.g., `${PersonDM, [/__meta/docRef]}`)
		 * that will be resolved at runtime with the actual source document reference.
		 */
		...
		/**
		 * Height of the relationship component as a CSS length string (e.g. "328" or "50vh").
		 * Only applies when the component is rendered inside a CustomScreenElement.
		 * Defaults to "328" if not specified.
		 */
	...
	export interface EditConfiguration {
		/**
		 * Reference to the candidate overview model for the edit DualPane.
		 */
		readonly availableItemsOverviewModel: string;
