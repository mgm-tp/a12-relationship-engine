...
test.describe("Brand bindings", () => {
	...
	// DualPaneSelection props.height = "50vh" -> 50% * 720 = 360
	...
	// TableList props.height = "40vh" -> 40% * 720 = 288
	...
	test("TableList", async ({ page }) => {
		...
		await candidates.locator(Selector.PAGINATION).getByRole("combobox").selectOption("5 / 50");

		await expect(candidates).toContainText("Ceb Shoes");
		await candidates.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Ceb Shoes" }).locator(Selector.BUTTON).click();
		...
		await expect(links.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Ceb Shoes" })).toBeVisible();
		// Column Document Model Reference should has data "Product-document"
		await expect(
			links.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Ceb Shoes" }).locator(Selector.TABLE_BODY_CELL).nth(5)
		).toHaveText("Product-document");

		await candidates.locator(Selector.PAGINATION).getByRole("combobox").selectOption("17 / 50");
		await expect(candidates).toContainText("Jad Kit");
		await candidates.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Jad Kit" }).locator(Selector.BUTTON).click();
		await expect(page.getByText("Additional properties")).toBeVisible();
		await page.getByLabel("Manufacturing Site").fill("www.running-shoes.com");
		await page.getByRole("button", { name: "OK" }).click();
		await expect(links.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Jad Kit" })).toBeVisible();
		// Column Document Model Reference should has data "Bundle-document"
		await expect(
			links.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Jad Kit" }).locator(Selector.TABLE_BODY_CELL).nth(5)
		...
		await expect(
			page.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Ceb Shoes" }).locator(Selector.TABLE_BODY_CELL).nth(4)
		).toContainText("anonymous");
		await expect(
			page.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Jad Kit" }).locator(Selector.TABLE_BODY_CELL).nth(4)
		).toContainText("anonymous");
		...
		await expect(
			page.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Ceb Shoes" }).locator(Selector.TABLE_BODY_CELL).nth(5)
		).not.toBeEmpty();
		await expect(
			page.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Jad Kit" }).locator(Selector.TABLE_BODY_CELL).nth(5)
		).not.toBeEmpty();
	});
});
