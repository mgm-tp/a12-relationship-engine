...
test.describe.skip("Standalone Relationship UI", () => {
	...
	test("Add link to existing document should work as the relationship in form", async ({ page }) => {
		...
		const candidate = candidates.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Adidas" });
		...
		const link = links.locator(Selector.TABLE_BODY_ROW).filter({ hasText: "Adidas" });
		...
		await expect(
			page
				...
				.filter({ hasText: /Running shoes.*110.*local/ })
		).toBeVisible();
	});
});
